...
module.exports = function (app) {
  ...
  apiRoutes.post("/login", async (req, res) => {
    ...
    try {
      ...
      res
        .status(200)
        .send({ msg: "Login successful.", email: emailOrUsername });
    } catch (err) {
  ...
  apiRoutes.post("/forget-password/send-otp", async (req, res) => {
    ...
    try {
      ...
      if (!user) {
        ...
        return res
          .status(404)
          .send({ msg: "User not found.", status: "false" });
      }
      ...
      console.log(
        `Generated OTP: ${otp} for emailOrUsername: ${emailOrUsername}`
      );
  ...
  apiRoutes.post("/user-dashboard", async (req, res) => {
    ...
    try {
      ...
      const flightBookings = await Flight.find({
        "passenger.email": emailOrUsername,
      });
  ...
  apiRoutes.get("/admin-panel", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .send({
          msg: "Error fetching admin panel data.",
          error: err.message && err,
        });
    }
  });
